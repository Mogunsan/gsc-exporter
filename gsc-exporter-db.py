...
db_con = pymysql.connect(user='root', password='mypassword',
                              host='localhost',
                              database='gscapi',
                              port=3306)
...
#GUI / Browser get & add Authentification-Code
...
df = pd.read_csv((mod_path / query_path).resolve())
for index, row in df.iterrows():
    ...
    if "rows" in gsc_answer_json:
        ...
        gsc_df.to_csv('rows'+startDate+'_bis_'+endDate+'.csv', mode = 'a', index = False, encoding='utf-8')
        print(index)
        print(row)

#Create the .csv and clean it up
gsc_rows_df = pd.read_csv('rows'+startDate+'_bis_'+endDate+'.csv')
gsc_rows_df.drop_duplicates(keep = 'first', inplace = True)
gsc_rows_df.to_csv('rows'+startDate+'_bis_'+endDate+'.csv',mode = 'w' , index = False, encoding='utf-8')

#Database Database, just living in a 
engine = sqlalchemy.create_engine('mysql+pymysql://root:mypassword@localhost:3306/gscapi')
gsc_rows_df.to_sql('gsc_daten_'+site_url+'_'+startDate+'_bis_'+endDate,con=engine,index=False,if_exists='append')

#Database - Add Primärschlüssel
add_primary_key_stmt = "ALTER TABLE `"+'gsc_daten_'+site_url+'_'+startDate+'_bis_'+endDate+"` ADD `id` INT(10) NOT NULL AUTO_INCREMENT FIRST, ADD PRIMARY KEY (`id`);"
try:
    with db_con.cursor() as cur:
        cur.execute(add_primary_key_stmt)
finally:
    db_con.close()

os.remove('rows'+startDate+'_bis_'+endDate+'.csv')
if os.path.exists('rows'+startDate+'_bis_'+endDate+'.csv'):
  os.remove('rows'+startDate+'_bis_'+endDate+'.csv')
else:
  print("Done btw")